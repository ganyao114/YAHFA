...
import lab.galaxy.yahfa.HookMethodResolver;
        ...
        for (HookEntity entity:hookEntityMap.values()) {
            if (entity.target != null && entity.hook != null) {
                if (entity.backup != null) {
                    HookMethodResolver.resolveMethod(entity.hook, entity.backup);
                }
                if (!HookMain.backupAndHookNative(entity.target, entity.hook, entity.backup)) {
                    throw new RuntimeException("Failed to hook " + entity.target + " with " + entity.hook);
                }
            }
        }
    }
